              ...
              if (matchedStock !== undefined) {
                ...
                convertedUserStocks.push({
                  ...
                  change: Number.parseFloat(
                    ((priceDiff / eachStock.oldPrice) * 100).toFixed(2)
                  ),
                  changeReaction: {
        ...
        .subscribe((_res: Partial<Stock>) => {
          ...
          if (price) {
            ...
            if (displayedStockIndex !== -1 && oldPrice !== undefined) {
              ...
              const changeResult = Number.parseFloat(
                (price - (oldPrice ?? foundStock.currentPrice)).toFixed(2)
              );
              const gainLossResult = Number.parseFloat(
                (
